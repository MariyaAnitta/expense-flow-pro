...
import { getCurrencySymbol, getSupportedCurrencies } from '../currencyService';
...
const SystemSettings: React.FC<SystemSettingsProps> = ({ settings, onUpdate }) => {
    ...
    const [reportingCurrency, setReportingCurrency] = useState(settings.reporting_currency || 'USD');
    ...
    useEffect(() => {
        ...
        setReportingCurrency(settings.reporting_currency || 'USD');
    }, [settings.audit_threshold, settings.reporting_currency]);
    ...
    return (
        <div className="max-w-4xl mx-auto space-y-8 animate-in slide-in-from-bottom-4 duration-500 pb-20">
            ...
            <div className="bg-white dark:bg-slate-900 p-10 rounded-[3rem] border border-slate-100 dark:border-slate-800 shadow-xl">
                ...
                <div>
                    <h4 className="text-sm font-black dark:text-white uppercase tracking-wider mb-2">Global Reporting Currency</h4>
                    <p className="text-xs font-medium text-slate-500 max-w-lg leading-relaxed">
                        Set the base currency for all company reports, audit thresholds, and compliance calculations.
                    </p>
                    <div className="mt-4 flex flex-wrap gap-2">
                        {getSupportedCurrencies().map(curr => (
                            <button
                                key={curr}
                                onClick={() => setReportingCurrency(curr)}
                                className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${reportingCurrency === curr
                                        ? 'bg-brand-600 text-white shadow-lg shadow-brand-600/20'
                                        : 'bg-white dark:bg-slate-900 text-slate-400 border border-slate-100 dark:border-slate-800 hover:border-brand-200'
                                    }`}
                            >
                ...
                <div className="border-t border-slate-100 dark:border-slate-800/60 my-8 pt-8">
                    <div className="flex flex-col md:flex-row md:items-start justify-between gap-6 mb-8">
                        <div>
                            ...
                            <p className="text-xs font-medium text-slate-500 max-w-lg leading-relaxed">
                                Any expense above this amount (converted to {reportingCurrency}) will require a verified receipt or invoice for reconciliation.
                            </p>
                        </div>
                    ...
                    <div className="flex flex-col md:flex-row gap-4">
                        <div className="flex-1 relative group">
                            <div className="absolute left-6 top-1/2 -translate-y-1/2 flex items-center gap-2">
                                <span className="text-xs font-black text-slate-400">{reportingCurrency} {getCurrencySymbol(reportingCurrency)}</span>
                            </div>
                            <input
                                type="number"
                                className="w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl py-5 pl-24 pr-8 text-xl font-black focus:outline-none focus:ring-4 focus:ring-brand-500/10 transition-all dark:text-white"
                                value={threshold}
